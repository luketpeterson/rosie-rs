...
//A wrapper around an EnginePtr
pub(crate) struct RawEngine(pub(crate) EnginePtr);

//Safety Note: Each [RawEngine] / EnginePtr object represents one rosie engine.  The pointer is
// unique and hasn't been shared.  A rosie engine contains a Lua state, so therefore isn't thread
// safe (i.e. it's not `Sync`), but multiple rosie engines play nicely together
...
impl RawEngine {
    ...
    pub fn lib_paths(&self) -> Result<Vec<PathBuf>, RosieError> {
        ...
        let result_code = unsafe { rosie_libpath(self.0, &mut paths_rosie_string) };
    ...
    //is safe.
    pub fn compile(&self, expression : &str, messages : Option<&mut RosieMessage>) -> Result<i32, RosieError> {
    ...
    pub fn import_expression_deps(&self, expression : &str, messages : Option<&mut RosieMessage>) -> Result<(), RosieError> {
        ...
        let result_code = unsafe { rosie_import_expression_deps(self.0, &expression_rosie_string, ptr::null_mut(), &mut err, &mut message_buf) };
    ...
    pub(crate) fn match_pattern<'input>(&self, pattern_id : i32, start : usize, input : &'input [u8]) -> Result<MatchResult<'input>, RosieError> {
    ...
    pub(crate) fn match_pattern_raw<'engine>(&'engine self, pattern_id : i32, start : usize, input : &[u8], encoder : &MatchEncoder) -> Result<RawMatchResult<'engine>, RosieError> {
        ...
        let result_code = unsafe{ rosie_match2(self.0, pattern_id, encoder.as_bytes().as_ptr(), &input_rosie_string, u32::try_from(start).unwrap(), u32::try_from(input_rosie_string.len()+1).unwrap(), &mut match_result, 0) };
    ...
    pub(crate) fn trace_pattern(&self, pattern_id : i32, start : usize, input : &str, format : TraceFormat, trace : &mut RosieMessage) -> Result<bool, RosieError> {
        ...
        let result_code = unsafe { rosie_trace(self.0, pattern_id, i32::try_from(start).unwrap(), format.as_bytes().as_ptr(), &input_rosie_string, &mut matched, &mut trace.0) };
...
/// 
/// **NOTE**: RosieEngines are not internally thread-safe and therefore don't implemnt [Sync].  You may create more than one RosieEngine in
/// order to use multiple threads.  If you need thread-portable engines or compiled patterns, use the alternative objects in [thread_portable].
/// 
/// Also there is a thread-local `default_engine` accessible through [take_thread_default_engine](Rosie::take_thread_default_engine) and [replace_thread_default_engine](Rosie::replace_thread_default_engine).
/// 
// INTERNAL NOTE: RosieEngine is just an Rc pointer around a RawEngine, in order to enable a Pattern taking
// a pointer to the engine, so that the API doesn't need to make users aware the engine exists unless they
// want to access the lower-level engine interfaces. 
//
// We make a clone of the Rc pointer when creating a compiled pattern, but externally, we don't want users
// to Clone RosieEngine because it breaks a conceptual model.  or example, if a user cloned an engine and
// changed a config parameter on a clone, it would be confusing when the setting also changed on the
// original engine.
pub struct RosieEngine(pub(crate) Rc<RawEngine>);
...
impl RosieEngine {
    ...
    pub fn new(messages : Option<&mut RosieMessage>) -> Result<Self, RosieError> {
        Ok(RosieEngine(Rc::new(RawEngine::new(messages)?)))
    }
    ...
    /// ```
    pub fn set_lib_paths<P: AsRef<Path>>(&mut self, new_paths : &[P]) -> Result<(), RosieError> {
        //Safety: changing the lib_paths doesn't affect any current state (i.e. compiled patterns)
        // It only affects future compilations.  Compiled patterns are the only place we retain
        // references to the engine
        #[allow(mutable_transmutes)]
        let raw_engine : &mut RawEngine = unsafe { std::mem::transmute(&(*self.0)) };

        raw_engine.set_lib_paths(new_paths)
    }
    ...
    pub fn set_mem_alloc_limit(&mut self, new_limit : usize) -> Result<(), RosieError> {
        //Safety: Changing the allocation limit inside the Rosie engine should not invalidate
        // any existing compiled patterns, which are the only allowed outstanding references
        #[allow(mutable_transmutes)]
        let raw_engine : &mut RawEngine = unsafe { std::mem::transmute(&(*self.0)) };

        raw_engine.set_mem_alloc_limit(new_limit)
    }
    ...
    pub fn compile(&self, expression : &str, messages : Option<&mut RosieMessage>) -> Result<Pattern, RosieError> {
        ...
        let pat_idx = self.0.compile(expression, messages)?;

        //NOTE: This is the only place a RosieEngine's Rc pointer gets cloned.  See the note around the
        // definition of RosieEngine
        Ok(Pattern{
            engine : RosieEngine(self.0.clone()),
            id : pat_idx    
        })
    }
    ...
    ///
    pub fn import_expression_deps(&self, expression : &str, messages : Option<&mut RosieMessage>) -> Result<(), RosieError> {
        self.0.import_expression_deps(expression, messages)
    }
    ...
    /// 
    pub fn load_pkg_from_str(&self, rpl_text : &str, messages : Option<&mut RosieMessage>) -> Result<RosieMessage, RosieError> {
        self.0.load_pkg_from_str(rpl_text, messages)
    }
    ...
    /// 
    pub fn load_pkg_from_file<P: AsRef<Path>>(&self, file_name : P, messages : Option<&mut RosieMessage>) -> Result<RosieMessage, RosieError> {
        self.0.load_pkg_from_file(file_name, messages)
    }
    ...
    /// 
    pub fn import_pkg(&self, pkg_name : &str, alias : Option<&str>, messages : Option<&mut RosieMessage>) -> Result<RosieMessage, RosieError> {
        self.0.import_pkg(pkg_name, alias, messages)
    }
}
